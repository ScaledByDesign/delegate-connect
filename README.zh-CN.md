...
[Connector SDK](https://github.com/oomol-lab/connector-sdk)、MCP 和 HTTP 为 AI Agent 提供 6,680 个可直接调用的预置 Action。
...
本仓库是 oomol 更大 connector catalog 的开源迁移版本。商业 SaaS 版已经支持 1,000+ 个 SaaS
provider；当前开源 catalog 已适配 643+ 个 provider，后续会随着工程迁移持续把更多集成整理成可维护的开源
provider definition 和 executor。

[oo CLI](https://github.com/oomol-lab/oo-cli) 对开源 runtime 的支持也正在从商业版迁移适配，目标是在
2026 年 7 月中旬可用。在这之前，可以先使用下面的 SDK、MCP、HTTP API、OpenAPI 和本地 Web 控制台路径。
...
- 面向 Agent 的调用入口，支持 [Connector SDK](https://github.com/oomol-lab/connector-sdk)、
  MCP、HTTP API、OpenAPI 和本地 Web 控制台；[oo CLI](https://github.com/oomol-lab/oo-cli) 正在做开源版本
  适配。
...
| [oo CLI](https://github.com/oomol-lab/oo-cli)               | 开源 runtime 支持正在从商业版迁移适配，目标是在 2026 年 7 月中旬可用。                   |
...
| [oomol](https://oomol.com/) | 被 OAuth 申请周期或上线时间卡住的团队 | 托管鉴权、运行时和 1,000+ provider catalog，并通过兼容接口保留后续私有化或自托管部署路径 |
...
如果业务需要马上上线，[oomol](https://oomol.com/) 提供商业 SaaS 版：先使用托管鉴权和运行时基础设施，并可访问更完整的 1,000+ provider catalog。剩余集成会随着工程迁移继续进入这个开源代码库，团队也可以保留后续迁移到私有化或自托管部署的空间。
